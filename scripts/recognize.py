...
4a. 海报是 7 列网格（week_start ~ week_end 各占一列），每个直播卡片严格归属其所在列的日期。
    卡片为斜角排版、列间容易视觉错位：输出前必须逐列核对（第1列=week_start，第2列=+1天……
    第7列=week_end），严禁所有条目整体错位一天（2026-09-14 当周曾整体提前一天）
4b. 某列若是「休息日」「勿忘国耻 铭记历史」等非直播内容块，该天 events 为空数组 []
5. "休息"字样对应 tag 填 "rest"
6. 时间统一为24小时制 "HH:MM"
6a. title 必须严格照抄卡片上的文字（含标点与省略号），不得改写或同音/形近替换
