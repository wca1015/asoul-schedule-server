...
python scripts/test_smoke.py          # 离线冒烟测试，21 项全绿即环境就绪
...
覆盖（21 项）：规则时间提取、事件 ID 生成、突击直播自动发布与去重、过期清理、数据校验、发布字段兜底、版本号单调递增（周程+突击）、归档扫描、录播回填匹配与准确率、联合直播不误配、直播预约卡片解析、直播间开播检测与日程窗过滤、已播突击并入周程表、突击回扫候选、反代密钥头携带、积压旧动态守卫、AI 提示词发布时间上下文与过期结果丢弃。全部离线，不请求网络、不调用 AI。
