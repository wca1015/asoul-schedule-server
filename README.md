...
- **积压旧动态守卫**：动态接口被风控中断数天后恢复时会一次性补拉大量旧动态；发布超过 24h 的动态直接跳过识别，识别结果开播时间超过 48h 的也丢弃（AI 曾把 9/15 旧公告里的「19:40 开始」误算为 9/21 突击）
- **周表识别（按列锚定）**：VLM 先逐字转抄 7 个日期列头、再逐列读取卡片，程序以列头日期为准重建日程（而非让模型自行推算网格）——针对 2026-09-14 / 09-21 两次「整体错位一天」事故的机制性修复；列头无法解析的列回退按列序推算
...
python scripts/test_smoke.py          # 离线冒烟测试，22 项全绿即环境就绪
...
1. 构建 Release APK：在 App 仓库执行 `gradlew assembleRelease`——
   构建脚本会自动产出发版产物 `app/build/outputs/apk/publish/zhijiang-calendar.apk`（文件名固定，无需改名）

   > ⚠️ 固定名必须为 ASCII：GitHub Releases 会把非 ASCII 资产名归一化为 `default.apk`（实测确认），
   > 会导致 OSS 清单里的 `apk_url` 404，因此不能使用中文文件名。

2. 上传到 GitHub Releases（资产名即上一步文件名）：

   ```powershell
   gh release create vX.Y app/build/outputs/apk/publish/zhijiang-calendar.apk `
...
- 动态接口风控中断后恢复 → 一次性补拉的积压动态中，发布超过 24h 的直接跳过识别（AI 以动态发布日为基准换算时间，避免旧文时间被误当"今天"）
...
覆盖（22 项）：规则时间提取、事件 ID 生成、突击直播自动发布与去重、过期清理、数据校验、发布字段兜底、版本号单调递增（周程+突击）、归档扫描、录播回填匹配与准确率、联合直播不误配、直播预约卡片解析、直播间开播检测与日程窗过滤、已播突击并入周程表、突击回扫候选、反代密钥头携带、积压旧动态守卫、AI 提示词发布时间上下文与过期结果丢弃、周表按列转换（列头锚定防错位）。全部离线，不请求网络、不调用 AI。
